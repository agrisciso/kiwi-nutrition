...
function PasswordGate({ onUnlock, t, lang, setLang }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.darkGreen, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'Inter',-apple-system,sans-serif" }}>
      {/* Lang switcher */}
      ...
      {/* Card */}
      <div style={{ background:C.cream, borderRadius:20, padding:"36px 28px", maxWidth:360, width:"100%", textAlign:"center", animation: shake ? "shake 0.4s ease" : "none" }}>
        {/* Icon */}
        <div style={{ width:80, height:80, borderRadius:20, background:C.darkGreen, margin:"0 auto 20px", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <NutritionIcon size={56} />
        </div>
        <div style={{ fontSize:11, color:C.gold, fontWeight:700, letterSpacing:"0.15em", textTransform:"uppercase", marginBottom:6 }}>AgriSci Solutions</div>
        <div style={{ fontSize:20, fontWeight:800, color:C.text, marginBottom:6 }}>{t.pwTitle}</div>
        <div style={{ fontSize:13, color:C.textMuted, marginBottom:28 }}>{t.pwSubtitle}</div>

        <input
          type="password" value={pw} placeholder={t.pwPlaceholder}
          onChange={e => { setPw(e.target.value); setError(false); }}
          onKeyDown={e => e.key === "Enter" && attempt()}
          style={{ width:"100%", padding:"12px 16px", borderRadius:10, border:`2px solid ${error ? C.red : C.creamDark}`, background:C.white, fontSize:16, color:C.text, outline:"none", boxSizing:"border-box", marginBottom:8, textAlign:"center", letterSpacing:"0.1em" }}
          onFocus={e => (e.target.style.borderColor = error ? C.red : C.gold)}
          onBlur={e => (e.target.style.borderColor = error ? C.red : C.creamDark)}
        />
        {error && <div style={{ color:C.red, fontSize:12, marginBottom:8, fontWeight:600 }}>{t.pwError}</div>}

        <button onClick={attempt} style={{ width:"100%", padding:"13px", borderRadius:10, background:C.darkGreen, color:C.gold, border:"none", fontSize:15, fontWeight:800, cursor:"pointer", letterSpacing:"0.05em", marginTop:4 }}>
          {t.pwBtn}
        </button>

        <div style={{ marginTop:24, fontSize:11, color:C.textMuted, lineHeight:1.8 }}>
          {t.pwFooter}{" "}
          <a href="https://agrisci-solutions.com/#tools" style={{ color:C.accent, fontWeight:600 }}>agrisci-solutions.com</a>
        </div>
      </div>
